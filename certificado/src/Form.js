...
export default class Form extends React.Component {
  ...
  _handleSubmit(e) {
    ...
    // Send data to parent
    this.props.onFormSubmit({email: email, cpf: cpf, isSpeaker: this.state.isSpeaker, year: e.target.year.value});
  }
            ...
            <div className="form-item">
              <select name="year" id="year">
                { this.state.years.map(year => {
                  return <option key={ year.toString() } value={ year }>{ year }</option>
                }) }
              </select>
            </div>
